...
ReturnValue evaluate_expression(const ASTNode *node, struct hashmap *scope, ReturnContext *ret_ctx) {
    ...
    case NODE_EXPR_BINARY: {
        ReturnContext left_eval_ctx = {
            .is_return = 0,
            .ret_val.type = RET_NONE
        };
        ReturnContext right_eval_ctx = {
            .is_return = 0,
            .ret_val.type = RET_NONE
        };
...
void execute_var_decl(const VarDeclNode *node, struct hashmap *scope, ReturnContext *ret_ctx) {
    ...
    // FIX: Use node->type for variable type instead of node->data.var_decl.type
...
void execute_func_call(const FuncCallNode *func_call, struct hashmap *scope, ReturnContext *caller_ret_ctx) {
    ...
    if (c_func) {
        ...
        for (int i = 0; i < c_func->param_count; i++) {
            ...
            case TYPE_DOUBLE_ARRAY:
                ...
                if (arg_vals[i].value.list_val.element_type != VAR_NUM) {
                    fprintf(stderr, "\nError: Argument %d to %s must be a list of nums\n", i + 1, func_call->name);
                    goto cleanup_and_exit;
                }
                args[i] = &arg_vals[i].value;
                break;
        ...
        case TYPE_STRING_ARRAY:
        case TYPE_DOUBLE_ARRAY:
            ret_out = malloc(sizeof(VariableValue));
            break;
        case TYPE_VOID:
            break;
        default:
        ...
        case TYPE_DOUBLE_ARRAY: {
            ...
            ret_val.type = RET_LIST;

            ret_val.value.list_val.element_type = vv->list_val.element_type;
            ret_val.value.list_val.nested_element_type = vv->list_val.nested_element_type;
            ret_val.value.list_val.is_nested = vv->list_val.is_nested;
            ret_val.value.list_val.head = deep_copy_list(vv->list_val.head);
            break;
        }
        case TYPE_VOID:
            ret_val.type = RET_NONE;
    ...
    if (func_call->arg_count != function->param_count) {
        fprintf(stderr, "Function %s expects %d arguments, but got %d\n",
            func_call->name,
            function->param_count, func_call->arg_count);
        exit(EXIT_FAILURE);
    }
...
void execute_return(const ReturnNode *node, struct hashmap *scope, ReturnContext *ret_ctx) {
    ...
    case RET_LIST:
        ...
        ret_ctx->ret_val.value.list_val = temp.ret_val.value.list_val;

        temp.ret_val.value.list_val.head = NULL;
        break;
    ...
    free_return_value(temp.ret_val.type, &temp.ret_val);
...
void execute_assignment(const AssignmentNode *node, struct hashmap *scope) {
    ...
    if (node->target_name == NULL && node->target_access != NULL) {
        ...
        Variable *
            var = get_variable(scope ? scope : variable_map, current->data.variable_access.name);
        if (!
            var) {
        ...
        VarType current_type =
            var->value.list_val.element_type;
        VarType nested_type =
            var->value.list_val.nested_element_type;
        bool is_nested =
            var->value.list_val.is_nested;
    ...
    Variable *
        var = get_variable(scope ? scope : variable_map, node->target_name);
    if (!
        var) {
        fprintf(stderr, "\nError: Variable '%s' not found for assignment.\n", node->target_name);
        exit(EXIT_FAILURE);
...
ReturnValue evaluate_variable_access(const VariableAccessNode *node, struct hashmap *scope, ReturnContext *ret_ctx) {
    ...
    Variable *
        var = get_variable(scope ? scope : variable_map, node->name);
    if (!
        var) {
