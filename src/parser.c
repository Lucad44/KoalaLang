...
ASTNode *parse_binary_expr(Parser *parser, ASTNode *left,
    const int min_prec) {
    const struct {
        TokenType token;
        BinaryOperator op;
...
ASTNode *parse_function_declaration(Parser *parser) {
    ...
    if (hashmap_get(function_map, &(Function) {
            .name = func_name
        }) != NULL) {
...
ASTNode *parse_function_call(Parser *parser) {
    ...
    node->data.func_call.module_name = module_name;
    return node;
}
...
ASTNode *parse_expression_statement(Parser *parser) {
    ...
    if (parser->current_token.type == TOKEN_DOT) {
        ...
        advance(parser);
        if (parser->current_token.type != TOKEN_IDENTIFIER) {
            fprintf(stderr, "Error: Expected variable name after '%s.'\n", module_name);
            free(module_name);
            exit(EXIT_FAILURE);
        }
        var_name = strdup(parser->current_token.lexeme);
    ...
    if (module_name == NULL && 
        (parser->current_token.type == TOKEN_OPERATOR_PLUS_PLUS ||
         parser->current_token.type == TOKEN_OPERATOR_MINUS_MINUS)) {
        
        PostfixOperator op = parser->current_token.type == TOKEN_OPERATOR_PLUS_PLUS 
                             ? OP_INC : OP_DEC;
        advance(parser);  

        if (parser->current_token.type != TOKEN_SEMICOLON) {
            fprintf(stderr, "Error: Expected ';' after postfix operation\n");
            free(var_name);
            exit(EXIT_FAILURE);
        }
        advance(parser);
        ...
        node->type = NODE_EXPR_POSTFIX;
        node->data.postfix_expr.op = op;
        node->data.postfix_expr.var_name = var_name;
        return node;
    }
    ...
    if (parser->current_token.type == TOKEN_LBRACKET) {
        ...
        access = safe_malloc(sizeof(ASTNode));
        access->type = NODE_VARIABLE_ACCESS;
        ...
        while (parser->current_token.type == TOKEN_LBRACKET) {
            ...
            if (parser->current_token.type != TOKEN_RBRACKET) {
                fprintf(stderr, "Error: Expected ']' after index expression\n");
                free_ast(idx);
                free_ast(access);
                exit(EXIT_FAILURE);
            }
    ...
    if (access != NULL || module_name != NULL || var_name != NULL) {
        if (parser->current_token.type != TOKEN_OPERATOR_EQUAL) {
            if (access != NULL) {
                fprintf(stderr, "Error: Expected '=' after indexed access\n");
                free_ast(access);
            } else {
...
ASTNode *parse_list_type(Parser *parser) {
    ...
    VarType nested_element_type = VAR_NUM;
...
ASTNode *parse_assignment(Parser *parser) {
    ...
    while (parser->current_token.type == TOKEN_LBRACKET) {
        ...
        if (first_index == NULL) {
            ...
            if (parent_access == NULL) {
                ...
                access_node->data.variable_access.name = NULL;
    ...
    if (parent_access != NULL) {
        ...
        node->data.assignment.target_name = NULL;
        node->data.assignment.index_expr = NULL;
        node->data.assignment.target_access = parent_access;
        node->data.assignment.value_expr = value_expr;
    } else {

        node->data.assignment.target_name = target_name;
        node->data.assignment.index_expr = first_index;
        node->data.assignment.target_access = NULL;
        node->data.assignment.value_expr = value_expr;
    }
...
ASTNode *parse_import(Parser *parser) {
    ...
    const Module *module = hashmap_get(module_map, &(Module) {
        .name = module_name
    });
    if (!module) {
